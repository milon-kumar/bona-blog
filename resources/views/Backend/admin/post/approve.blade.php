                    ...
                    <div class="col-sm-12">
                        <div class="card mb-0">
                            <div class="card-header">
                                ...
                                <a href="{{route('admin.post.create')}}" class="float-right btn btn-success btn-sm">Add Post</a>
                            </div>
                            <div class="card-body">
                                    ...
                                    <table class="datatable table table-stripped mb-0">
                                        ...
                                        @foreach($posts as $key => $post)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{route('admin.approve',$post->id)}}" onclick="event.preventDefault();
                                                        if (confirm('Are you sure to approve this post')){
                                                        document.getElementById('approveFrom'+{{$post->id}}).submit();
                                                        }
                                                        "class="btn btn-warning btn-sm">Approve</a>
                                                    <form style="display: none;" id="approveFrom{{$post->id}}" action="{{route('admin.approve',$post->id)}}" method="post">
                                                        @csrf
                                                        @method('PUT')
                                                    </form>
                                                    ...
                                                    <a href="{{route('admin.category.destroy',$post->id)}}" onclick="event.preventDefault();
                                                        if (confirm('Are you sure to delete this ')){
                                                        document.getElementById('deleteFrom'+{{$post->id}}).submit();
                                                        }
                                                        "  class="btn btn-danger btn-sm">Delete</a>
                                                    <form id="deleteFrom{{$post->id}}" action="{{route('admin.post.destroy',$post->id)}}" method="post">
                                                        @csrf
                                                        @method('DELETE')
                                                    </form>
                                                </td>
